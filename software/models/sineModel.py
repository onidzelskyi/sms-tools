...
def sineModelMultiRes(x, fs, W_list, N_list, t, B_list):
    """
    Analysis/synthesis of a sound using the multi-resolution sinusoidal model, without sine tracking
    :param x: input array sound.
    :type x: numpy array
    :param fs: input sound' frequency
    :type fs: unsigned int
    :param W: array of three analysis windows w1, w2, and w3.
    :type W: list of numpy arrays
    :param N: array of sizes of complex spectrum, N1, N2, and N3.
    :type N: list of unsigned ints
    :param t: threshold in negative dB
    :type t: negative int
    :param B: array fo three bands: B1, B2, and B3.
    :type B: list of unsighed ints
    returns y: output array sound
    :rtype: numpy array
    """
    z = np.zeros(x.size)

    for w, N, B in zip(W_list,N_list,B_list):
        hM1 = int(math.floor((w.size + 1) / 2))  # half analysis window size by rounding
        hM2 = int(math.floor(w.size / 2))  # half analysis window size by floor
        Ns = 512  # FFT size for synthesis (even)
        H = Ns / 4  # Hop size used for analysis and synthesis
        ...
        while pin < pend:  # while input sound pointer is within sound
            ...
            # ipfreq contains frequencies
            # We subtract from ploc frequencies that are out of band range
            ploc = ploc[(B[0] < ipfreq) & (ipfreq < B[1])]

            # Next, recalculate iploc, ipmag, ipphase, and ipfreq
            # for new array of peaks
            iploc, ipmag, ipphase = UF.peakInterp(mX, pX, ploc)  # refine peak values by interpolation
            ipfreq = fs * iploc / float(N)  # convert peak locations to Hertz
